...
이 10개 정책은 DB Core 검증 과정(Round 1~11B)에서 실제로 확정된 사실이며,
API 설계 전체에 우선 적용된다.
...
10. **`is_closed = TRUE` 이후 정산 이슈 처리는 허용하되, 원본 거래 직접
    수정은 금지한다(DL-033).** Closed Formula에서 수량·단가·환율·참여자·
    기존 snapshot·기존 payment/invoice 금액·share 직접 변경 및 Version/
    Share/Formula 일반 수정은 API가 거부해야 한다. 추가 payment record,
    payment cancel 후 재입금, Settlement API 경유 schedule 추가, 조건부
    invoice sync, `audit_logs` 정산 메모는 Settlement allowlist로만 허용한다.
    Reopen, Close undo, Adjustment Formula entity는 MVP에서 제공하지 않는다.
...
## 1.2a Formula formula_no 조회 (보조 endpoint)

| 항목 | 내용 |
|---|---|
| Method | `GET` |
| Endpoint | `/api/v1/formulas/by-formula-no/{formula_no}` |
| 목적 | DB가 채번한 `formula_no`로 Formula 단건 조회 (조회 전용) |

**Response Body**: §1.2와 동일 (`quantity`, `unit`, `item_id`, `closed_at` 포함).

- **DB 사용 테이블/View**: `formulas` (SELECT, `formula_no` UNIQUE)
- **핵심 비즈니스 규칙**:
  - **formula_no 생성/수정 API가 아니다.** 핵심 정책 1 — Formula 생성
    요청에 `formula_no` 입력 금지, 이 endpoint는 **조회만** 수행한다.
  - §1.2 `GET /formulas/{formula_id}` (UUID)가 primary lookup path이다.
    본 endpoint는 업무 번호(`FM-YYMM-#####`) 기반 **보조 조회** path이다.
- **구현 상태**: HTTP Route 완료.
...
- **실패 케이스**:
  - `is_closed = TRUE`인 Formula 수정 시도 → API가 명시적으로 거부해야
    한다(409 Conflict, API Layer 책임). **정산 메모는 1.4가 아니라
    8.5 Settlement API(`audit_logs`)로 기록한다.**
  - `quantity`/환율/단가 변경 시도 → 400 (7.1 Version API 안내). Closed
    Formula에서는 7.1 자체도 거부(8.5 참조).
...
- **실패 케이스**:
  ...
  - 동일 Formula에 시작점/종료점이 이미 존재하는데 추가 시도 → UNIQUE
    INDEX 위반.
  - `is_closed = TRUE`인 Formula → **409 거부(DL-033)**.
...
- **Closed Formula 정책 (DL-033)**: `is_closed = TRUE`이면 **일반 3.1
  경로는 거부(409)**. 종결 후 발견된 예정금 보정은 **8.5 Settlement API**
  (`POST .../settlement/payment-schedules`)에서만 허용한다.
...
- **핵심 비즈니스 규칙**:
  ...
  - **`payment_schedule_id`를 생략할 수 있다**(예정 없이 입금되는
    ...
    실무자가 추후 매칭할 수 있다.
- **Closed Formula 정책 (DL-033)**: `is_closed = TRUE`에서도 **추가
  payment record 등록(append-only)은 허용**한다. Settlement allowlist
  경로(3.2 또는 8.5 settlement alias)로 처리한다. 기존 record의
  `actual_amount` 직접 UPDATE는 금지 — cancel(3.3) 후 신규 record로 보정.
- **실패 케이스**:
...
- **Closed Formula 정책 (DL-033)**: `is_closed = TRUE`에서도 **payment
  record cancel 및 cancel 후 재입금(3.2) 조합은 허용**한다(TEST-002 패턴).
...
- **핵심 비즈니스 규칙**: `receive_rate`/`payment_rate`는 예정금액이
  ...
  100 초과 가능, DB가 막지 않음).
- **구현 상태 — HTTP Route 완료**:
  - `getFormulaReceivablePayable` — 동일 View 기반 **8필드 subset** 반환.
  - §9.1 `GET .../kpi/confirmed`는 동일 View의 **full KPI response**
    (formula_id, status 필드 등 추가). 두 path는 목적이 다르며 공존한다.
...
- **구현 상태 — HTTP Route 완료** (read-only aggregate 조회).
- **`syncFormulaInvoiceStatus`는 HTTP GET으로 노출하지 않는다.** §4.3은
  create/update invoice 후 Service 내부 sync이며 side effect(UPDATE)를
  수반한다. §4.2와 목적·Response·side effect가 다르다.
...
**HTTP 노출**: 없음. `syncFormulaInvoiceStatus`는 mutation 후 Service
내부에서만 호출한다. GET aggregate 조회는 §4.2를 사용한다.
...
- 4.1(계산서 등록), 계산서 status 변경, 계산서 취소 — 이 작업들은
  동기화를 트리거해야 한다.
- **Closed Formula 조건부 sync (DL-033)**: `is_closed = TRUE`인 Formula는
  `derived_invoice_status = 'AMOUNT_MATCHED'`일 때**만** 2번 UPDATE를
  수행한다. 그 외 derived 값(`AMOUNT_MISMATCHED`, `REVISION_REQUIRED` 등)이면
  `formulas.invoice_status` UPDATE를 **건너뛴다** — `chk_closed_requires_all_completed`
  CHECK 위반을 방지한다. 개별 `formula_invoices.status` 변경은 허용하되,
  formula 컬럼 sync는 조건부로만 반영한다.
...
- 이 동기화를 누락하면 `expected_net_profit`이 실제 셰어 변경을
  반영하지 못한 채로 남는다.
- **Closed Formula 정책 (DL-033)**: `is_closed = TRUE`인 Formula에 대한
  Share CRUD(6.1~6.3)는 **거부(409)** — 원본 거래 변경으로 분류.
...
- **핵심 비즈니스 규칙 — version_no 충돌 시 API 재시도 정책 (필수)**:
  ...
  5. Version INSERT가 실패하면 그 Version의 `id` 자체가 생성되지
     ...
     이 연쇄를 DB 레벨에서 강제).
- **Closed Formula 정책 (DL-033)**: `is_closed = TRUE`인 Formula에 대한
  Version 생성(7.1)은 **거부(409)** — 원본 거래 변경으로 분류.
...
- **Canonical path**: `{formula_id}` + `{version_no}` (integer) composite lookup.
- **구현 상태 — HTTP Route 완료**.

## 7.3a 버전 상세 (UUID supplemental)

| 항목 | 내용 |
|---|---|
| Method | `GET` |
| Endpoint | `/api/v1/versions/{version_id}` |
| 목적 | UUID로 Version 단건 조회 (supplemental) |

- §7.3 `{formula_id}/{version_no}`가 **canonical** path이다.
- 본 endpoint는 list/create 응답의 internal `version_id`(UUID) 기반
  **보조 조회** path이다. formula 소속 검증은 §7.3 canonical path를
  우선 사용한다.
- **구현 상태**: HTTP Route 완료 (supplemental).
...
**Reopen (`is_closed TRUE → FALSE`)도 MVP에서 제공하지 않는다.**
DB가 기술적으로 허용하더라도 API Layer가 명시적으로 거부한다(8.4와
동일 — 엔드포인트 미제공).

**Adjustment Formula top-level entity도 MVP에서 제공하지 않는다.**
종결 후 계약 조건 자체 변경이 필요한 경우 V2에서 별도 설계·승인 후
검토한다. MVP는 동일 `formula_id` Settlement ledger 보정으로 처리한다.

## 8.5 Closed Formula Settlement Policy — **[DL-033 확정]**

종결(`is_closed = TRUE`)은 **원본 거래 동결 + 정산 ledger append-only
보정**을 의미한다. 6개 status 컬럼은 종결 시점 COMPLETED/
AMOUNT_MATCHED 스냅샷으로 DB CHECK에 의해 동결된다.

### 금지 (normal update paths — API가 409/400으로 거부)

| 대상 | 금지 작업 |
|---|---|
| Formula | 1.4 일반 수정, 8.1 취소 |
| Version / Share | 7.1 Version 생성, 6.x Share CRUD |
| Participant | 2.3 참여자 추가·변경·삭제 |
| Payment | 기존 record `actual_amount` UPDATE |
| Invoice | 기존 invoice 금액 필드 UPDATE |
| Snapshot | 기존 `formula_calculation_snapshots` UPDATE |

### 허용 (Settlement allowlist)

| Method | Endpoint | 목적 |
|---|---|---|
| `POST` | `/api/v1/formulas/{formula_id}/payment-records` | 추가 입출금 record (3.2) |
| `PATCH` | `/api/v1/payment-records/{record_id}/cancel` | record cancel (3.3) |
| `POST` | `/api/v1/formulas/{formula_id}/settlement/payment-schedules` | 종결 후 schedule 추가 (3.1 Settlement 전용) |
| `PATCH` | `/api/v1/invoices/{invoice_id}/status` | 개별 계산서 status 변경 + 4.3 조건부 sync |
| `POST` | `/api/v1/formulas/{formula_id}/settlement/notes` | 정산 메모/이슈 → `audit_logs` INSERT |
| `GET` | 3.4, 9.1, 9.3 | 미수/미지급·확정 KPI 조회 |

**Request Body (settlement/notes)**
```json
{
  "note": "string",
  "issue_type": "string",
...
- **DB 사용 테이블/View**: `audit_logs` (INSERT)
- **핵심 비즈니스 규칙**:
  - `table_name = 'formulas'`, `record_id = formula_id`,
    `action = 'SETTLEMENT_NOTE'` (또는 동등한 고정 action 값).
  - `new_data` JSONB에 `note`, `issue_type`, `changed_by` 포함.
  - Formula 1.4 PATCH로 정산 메모를 남기지 않는다.

### MVP 제외

- Close undo, Reopen, Adjustment Formula entity — 8.4 및 DL-033 참조.
...
- **구현 상태 — HTTP Route 완료** (full KPI response). §3.4
  `receivable-payable` alias는 동일 View의 8필드 subset이다.
...
| Method | `GET` |
| Endpoint | `/api/v1/formulas/{formula_id}/receivable-payable` |
| 목적 | 미수/미지급 8필드 subset 조회 (§3.4 canonical path) |

- **구현 상태 — HTTP Route 완료**. §9.1 `kpi/confirmed`는 full response.
...
| 8 | Closed Formula Settlement Policy | **확정 (8.5, DL-033)** | 종결 후 정산 ledger 보정 허용, 원본 거래 수정 금지. Reopen/Close undo/Adjustment Formula MVP 제외. |
