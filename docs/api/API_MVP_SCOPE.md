...
2. is_closed = TRUE인 Formula의 일반 속성 수정 — Close 이후 모든
   수정은 거부.
3. 이미 is_canceled = TRUE인 payment record의 재취소 요청 — 409.
4. Cancel Undo / Close Undo — 엔드포인트 미제공.
5. Formula 부분 취소 — 8.1은 전체 취소만 처리.
