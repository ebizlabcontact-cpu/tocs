...
| Field | Value |
|-------|--------|
| **Version** | v1.5.8 (Specification + **screen wireframe** — documentation only) |
...
**Screen wireframe:** **§15** (normative layout). Step order in §3 remains logical draft; wireframe steps 1–6 match §4.
...
| **진행률 표시** | Step index `N / 6` + progress bar |
...
**Footer (all steps):** `[ 이전 ]` `[ 다음 ]` — Step 6: `[ 이전 ]` `[ Formula 생성 ]`

---

### 15.2 Step wireframes (desktop)
...
│ Step 6 — 검토 (mandatory)                                               │
├────────────────────────────────────────────────────────────────────────┤
│ ▼ Formula Summary     item │ qty │ unit │ memo                         │
│ ▼ 참여자              table (Step 2 + roles)                           │
│ ▼ 금액                totals │ Estimated Net Profit (저장 전 예상치)      │
│ ▼ 예정 입출금         schedule list or “없음”                           │
│ ▼ 물류                summary or “없음”                                 │
│ ▼ Share (optional)    [ §12 A Pending — hide or empty unless approved ] │
│ ▼ Invoice             [ §12 B Pending — hide or empty unless approved ] │
├────────────────────────────────────────────────────────────────────────┤
│                              [ 이전 ]  [ Formula 생성 ]                 │
└────────────────────────────────────────────────────────────────────────┘
```
...
| Section | Source |
|---------|--------|
| Formula Summary | Step 1 |
| 참여자 | Step 2 |
| 금액 | Step 3 |
| 예정 입출금 | Step 4 |
| 물류 | Step 5 |
| Share optional | §12 **Pending** — wireframe slot only |
| Invoice | §12 **Pending** — wireframe slot only |

**Confirm:** triggers §7 commit sequence — **no** backend write before click.

---

### 15.3 완료 후 (post-create)
...
### 15.4 Mobile wireframe

**Rule:** One step **full screen** at a time; header (§15.1) sticky; fields **vertical stack**; preview blocks below inputs.
...
Same step order and validation as desktop.

---

### 15.5 Wireframe forbidden
